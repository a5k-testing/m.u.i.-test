...
describe('run() input validation', () => {
  ...
  describe('both apkDirs and apkFiles undefined', () => {
    let msg;
    before(async () => { msg = await runError({ core: {} }); });
    it('run(): throws when both apkDirs and apkFiles are undefined', () => { assert.ok(msg !== null); });
    it('run(): error message mentions apkDirs or apkFiles', () => { assert.ok(msg.includes('apkDirs') || msg.includes('apkFiles')); });
  });

  // apkFiles set but empty → throw
  describe('apkFiles set but empty', () => {
    let msg;
    before(async () => { msg = await runError({ core: {}, apkFiles: [] }); });
    it('run(): throws when apkFiles is set but empty', () => { assert.ok(msg !== null); });
    it('run(): error message mentions apkFiles', () => { assert.ok(msg.includes('apkFiles')); });
  });
...
describe('run() no processed APKs', () => {
  ...
  describe('all APK dirs nonexistent', () => {
    ...
    it('run(): no processed APKs exit code is set', () => { assert.ok(capturedExitCode !== undefined); });
    it('run(): no processed APKs exit code is not 1 or 2', () => { assert.ok(capturedExitCode !== 1 && capturedExitCode !== 2); });
  });
});
