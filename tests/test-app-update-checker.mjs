...
test('SKIP_LIST', () => {
  assert.strictEqual(SKIP_LIST.size, 4, 'SKIP_LIST has 4 entries');
  ...
  assert.strictEqual(filtered.length, 3, 'SKIP_LIST filter: 3 of 7 APKs kept');
  assert.strictEqual(filtered[0].fileName, 'FakeStore.apk',
    'SKIP_LIST filter: FakeStore.apk (no suffix) kept');
  assert.strictEqual(filtered[1].fileName, 'GmsCore.apk',
    'SKIP_LIST filter: GmsCore.apk (no suffix) kept');
  assert.strictEqual(filtered[2].fileName, 'Something.apk',
...
test('REPO_CACHE_TTL_MS', () => {
  assert.strictEqual(REPO_CACHE_TTL_MS, 7 * 24 * 60 * 60 * 1000,
...
test('index-v2.json URL construction', () => {
  const indexUrls = repos.map(b => `${b}/index-v2.json`);

  assert.strictEqual(indexUrls[0], 'https://repo.microg.org/fdroid/repo/index-v2.json',
    'microG: /index-v2.json appended correctly');
  assert.strictEqual(indexUrls[1], 'https://f-droid.org/repo/index-v2.json',
    'F-Droid: /index-v2.json appended correctly');
  assert.strictEqual(indexUrls[2], 'https://apt.izzysoft.de/fdroid/repo/index-v2.json',
...
test('fetchUrl: HTTPS enforcement', () => {
  ...
  assert.strictEqual(MAX_REDIRECTS, 3, 'Max redirects is 3');
...
test('parseRepoV2', () => {
  ...
  // Basic map size
  assert.strictEqual(fdroidApps.size, 4, 'F-Droid: parsed 4 packages');
  assert.strictEqual(microgApps.size, 1, 'microG: parsed 1 package');

  // latestVc / latestVn
  {
    const np = fdroidApps.get('org.schabi.newpipe');
    assert.strictEqual(np.latestVc, 1000, 'NewPipe: latestVc = 1000 (newest version wins)');
    assert.strictEqual(np.latestVn, '0.25.0', 'NewPipe: latestVn = 0.25.0');
  }

  // byCert — highest versionCode per cert
  {
    const np = fdroidApps.get('org.schabi.newpipe');
    assert.strictEqual(np.byCert[CERT_NEWPIPE].vc, 1000,
...
test('checkApks (matching loop)', () => {
  ...
  assert.strictEqual(results[0].tableRow[0], 'priv-app/FDroidPrivilegedExtension.apk',
    'FDroid priv: tableRow[0] shows relPath');
  assert.strictEqual(results[0].tableRow[2], 'f-droid',
  ...
  assert.strictEqual(results[0].noticeLine, null,
    'FDroid priv: no notice for UP TO DATE');
  assert.strictEqual(results[0].warningLine, null,
    'FDroid priv: no warning for UP TO DATE');
  assert.strictEqual(results[0].checkFailedLine, null,
  ...
  assert.strictEqual(results[1].tableRow[0], 'app/NewPipe.apk',
    'NewPipe: tableRow[0] shows relPath (app/NewPipe.apk)');
  assert.strictEqual(results[1].noticeLine, null,
  ...
  assert.strictEqual(results[1].checkFailedLine, null,
  ...
  assert.strictEqual(results[2].tableRow[0], 'priv-app/GmsCore.apk',
    'GmsCore (microG cert): tableRow[0] shows relPath');
  assert.strictEqual(results[2].tableRow[2], 'microg',
  ...
  assert.strictEqual(results[2].warningLine, null,
    'GmsCore (microG cert): no warning for UPDATE AVAILABLE');
  assert.strictEqual(results[2].checkFailedLine, null,
  ...
  assert.strictEqual(results[4].tableRow[0], 'app/NewPipeFork.apk',
    'NewPipeFork: tableRow[0] shows relPath');
  assert.strictEqual(results[4].tableRow[2], 'f-droid',
    'NewPipeFork: tableRow repo = f-droid (compact label)');
  assert.strictEqual(results[4].tableRow[3], `${ICON.DIFFERENT_SIGNER} DIFFERENT SIGNER`,
    'NewPipeFork: tableRow status has 🔐 DIFFERENT SIGNER');
  assert.strictEqual(results[4].noticeLine, null,
    'NewPipeFork: no notice for DIFFERENT SIGNER');
  assert.strictEqual(results[4].warningLine, null,
    'NewPipeFork: no warning for DIFFERENT SIGNER');
  assert.strictEqual(results[4].checkFailedLine, null,
  ...
  assert.strictEqual(results[5].tableRow[0], 'app/AuroraServices.apk',
    'AuroraServices: tableRow[0] shows relPath');
  assert.strictEqual(results[5].tableRow[3], `${ICON.NOT_IN_REPO} NOT IN REPO`,
    'AuroraServices: tableRow status has ❓ NOT IN REPO');
  assert.strictEqual(results[5].tableRow[2], '-',
    'AuroraServices: tableRow repo = -');
  assert.strictEqual(results[5].noticeLine, null,
    'AuroraServices: no notice for NOT IN REPO');
  assert.strictEqual(results[5].warningLine, null,
    'AuroraServices: no warning for NOT IN REPO');
  assert.strictEqual(results[5].checkFailedLine, null,
  ...
  assert.strictEqual(results[6].warningLine, null,
    'FakeStore: no warning for UPDATE AVAILABLE');
  assert.strictEqual(results[6].checkFailedLine, null,
  ...
  assert.strictEqual(results[7].tableRow[3], `${ICON.DIFFERENT_SIGNER} DIFFERENT SIGNER`,
    'GmsCoreOther: tableRow status has 🔐 DIFFERENT SIGNER');
  assert.strictEqual(results[7].noticeLine, null,
    'GmsCoreOther: no notice for DIFFERENT SIGNER');
  assert.strictEqual(results[7].warningLine, null,
    'GmsCoreOther: no warning for DIFFERENT SIGNER');
  assert.strictEqual(results[7].checkFailedLine, null,
...
test('checkApks (logEntry for DIFFERENT SIGNER and NOT IN REPO)', () => {
...
test('checkApks (updateInfo field)', () => {
  // Non-UPDATE_AVAILABLE entries have updateInfo = null
  assert.strictEqual(results[0].updateInfo, null, 'UP TO DATE: updateInfo is null');
  assert.strictEqual(results[1].updateInfo, null, 'LOCAL NEWER: updateInfo is null');
  assert.strictEqual(results[4].updateInfo, null, 'DIFFERENT SIGNER: updateInfo is null');
  assert.strictEqual(results[5].updateInfo, null, 'NOT IN REPO: updateInfo is null');
  ...
  assert.strictEqual(results[2].updateInfo.relPath,   'priv-app/GmsCore.apk',
    'GmsCore updateInfo: relPath');
  assert.strictEqual(results[2].updateInfo.package,   'com.google.android.gms',
    'GmsCore updateInfo: package');
  assert.strictEqual(results[2].updateInfo.repo,      'microg.org',
    'GmsCore updateInfo: repo');
  assert.strictEqual(results[2].updateInfo.localVn,   '22.0',
    'GmsCore updateInfo: localVn');
  assert.strictEqual(results[2].updateInfo.localVc,   220000000,
    'GmsCore updateInfo: localVc');
  assert.strictEqual(results[2].updateInfo.repoVn,    '23.9.13',
    'GmsCore updateInfo: repoVn');
  assert.strictEqual(results[2].updateInfo.repoVc,    230913000,
    'GmsCore updateInfo: repoVc');
  assert.strictEqual(results[2].updateInfo.url,
    'https://repo.microg.org/fdroid/repo/com.google.android.gms_230913000.apk',
    'GmsCore updateInfo: url');
  assert.strictEqual(results[2].updateInfo.repoFileSha256, SHA256_MICROG_GMS,
...
test('checkApks (tableRow <br> preserved for workflow)', () => {
...
test('update-info.dat format', () => {
  ...
  assert.strictEqual(datContent.split('\n').length, 3,
...
test('update-info.dat field validation', () => {
...
test('checkApks (CHECK_FAILED)', () => {
  ...
  assert.strictEqual(checkFailedResults[0].noticeLine, null,
    'ZeroLocalVc: no noticeLine when CHECK FAILED');
  assert.strictEqual(checkFailedResults[0].warningLine, null,
  ...
  assert.strictEqual(checkFailedResults[2].noticeLine, null,
    'NullRepoVc: no noticeLine when CHECK FAILED');
  assert.strictEqual(checkFailedResults[2].warningLine, null,
...
test('toPosix Windows path simulation', async () => {
    ...
    try {
      ...
      // Verify mock state
      assert.strictEqual(process.platform, 'win32', 'toPosix (win): process.platform mocked to win32');
      assert.strictEqual(path.sep,        '\\',     'toPosix (win): path.sep mocked to backslash');
      assert.strictEqual(path.win32.sep,  '\\',     'toPosix (win): path.win32.sep unchanged');
      assert.strictEqual(path.posix.sep,  '/',      'toPosix (win): path.posix.sep unaffected by mock');
      assert.strictEqual(path.join('C:', 'Users'),               'C:\\Users',   'toPosix (win): path.join uses win32');
      assert.strictEqual(path.dirname('C:\\Windows\\System32'), 'C:\\Windows', 'toPosix (win): path.dirname uses win32');
      ...
      assert.strictEqual(normDirs[0],  'C:/workspace/zip-content', 'toPosix (win): dir path 1 normalized');
      assert.strictEqual(normDirs[1],  'D:/apks/release',          'toPosix (win): dir path 2 normalized');
      assert.strictEqual(normFiles[0], 'C:/workspace/app.apk',     'toPosix (win): file path 1 normalized');
      assert.strictEqual(normFiles[1], 'D:/other/test.apk',        'toPosix (win): file path 2 normalized');
    } finally {
      await unmockWindows();
    }
});
